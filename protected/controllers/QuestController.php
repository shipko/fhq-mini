...
//	}
    ...
    public function actionList()
    {
        ...
        foreach ($section as $value) {
            ...
            $count_passed = count($value->count);

            // False - return without null values;
            $arr = $value->getAttributes(false);
            $arr['section'] = $value->stitle->getAttributes(false);
            $arr['count'] = $count_passed;
            $arr['passed'] = $passquest;
            $array[] = $arr;
        }
...
//    }
    ...
    public function actionPass()
    {
        ...
        // Fuck hellman
        $attempts = Attempts::model()->find('user=:user_id AND quest=:quest_id AND time > :time', array(
            ':user_id' => Yii::app()->params->user['user_id'],
            ':quest_id' => $id,
            ':time' => (time() - 5)
        ));
        ...
        if ($success) {
            ...
            $user_quest = UserQuest::model()->with(array(
                'quest_score' => array(
                    'select' => 'score'
                )

            ))->findAll('user=:user_id AND end_time > 0', array(
                    ':user_id' => Yii::app()->params->user['user_id']
                )
            );
            $rating = 0;
            foreach($user_quest as $value) {
                $q = $value->quest_score->getAttributes(false);
                $rating += $q['score'];
            }

            // Обновляем рейтинг пользователя

            Users::model()->updateByPk(Yii::app()->params->user['user_id'], array('rating' => $rating));
    ...
    public function actionCheck() {
        $users = Users::model()->findAll(array('select'=>'id'));

        foreach($users as $value) {
            $a = $value->getAttributes(false);
            ...
            Users::model()->updateByPk($a['id'], array('rating' => $rating));


        }
        Message::Success(array('id' => 'ok'));
    }
